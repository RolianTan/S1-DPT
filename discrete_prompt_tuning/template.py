class GenerationTemplate:
    ...
    def fill(self, full_demo='', input='', output='', soft_prompt=''):
        """
        Fills in the template with the given values.
        """
        return self.template.replace('[full_DEMO]', full_demo).replace(
            '[INPUT]', input).replace('[OUTPUT]', output).replace('[HINT]', soft_prompt)
...
#             '[full_DEMO]', full_demo).replace('[INPUT]', input).replace('[OUTPUT]', '')
#
...
class EvalTemplate:
    ...
    def fill(self, count, prompt='', full_demo='', input='', output=''):
        output = '[OUTPUT: positive or negative]'
