...
from discrete_prompt_tuning import llm
...
def get_eval_method(eval_method):
    ...
    if eval_method == 'sample':
        from discrete_prompt_tuning.evaluation import sample
        return sample.sample_evaluator
    elif eval_method == 'bandits':
        from discrete_prompt_tuning.evaluation import bandits
        return bandits.bandits_evaluator
    else:
        raise ValueError('Invalid evaluation method.')

def evalute_prompts(prompts, eval_template, eval_data, demos_template, demo_data, save_path, eval_method, config):
    """
    ...
    return eval_method(prompts, eval_template, eval_data, demos_template, demo_data, config, save_path)
