import random
from discrete_prompt_tuning import generate, evaluate, config, template, data, llm

# PART update to refine gen template
def get_simple_prompt_gen_template(prompt_gen_template, prompt_gen_mode):
    if prompt_gen_template is None:
        if prompt_gen_mode == 'forward':
            prompt_gen_template = "I gave my students a list of less-intepretable/incomplete hint text/keywords that are crucial to complete the folowing input-output task. Based on the hints, students should try to understand and find the relationship between these hints and task, so that they can provide a clear, general and interpretable step by step instruction that contains all the information and words in the hinds. This instruction should accurately complete the following input-output task:\n\n[full_DEMO]\n\n Please provide me an instruction with hint '\n\n[HINT]\n\n' that follows mentioned requirements (only provide me the instruction, do not include other content like explaination/examples). The instruction was: [APE]"
...
def simple_dpt(dataset_train,
    ...
    conf = config.simple_config(
        eval_model, prompt_gen_model)
    return find_prompts(eval_template, demos_template, dataset_train, dataset_eval, dataset_demo, conf, soft_prompt, save_path, prompt_gen_template=prompt_gen_template)
...
def find_prompts(eval_template,
    ...
    prompts = generate.generate_prompts(
        prompt_gen_template, demos_template, prompt_gen_data, soft_prompt, conf['generation'])
    ...
    res = evaluate.evalute_prompts(prompts, eval_template, eval_data, demos_template, demo_data, save_path, conf['evaluation']['method'], conf['evaluation'])
...
def get_generation_query(eval_template,
    ...
    for _ in range(num_query):
        ...
        queries.append(generate.get_query(prompt_gen_template,
                                          demos_template, subsampled_data, soft_prompt))
